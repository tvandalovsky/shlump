...
// num_bits: (((-1.44)*FALSE_POSITIVE_PROB.log2())*10.0) as i32
    pub fn printer(&mut self) {
        println!("{}: num_hash {}, num_elements {}, bit_vec len: {}", self.name, self.num_hash, self.num_elements, self.bit_vec.len());
        println!("optimal bit num: {}", -1.0 * (self.num_elements as f64 * FALSE_POSITIVE_PROB.ln()) / f64::powf(TWO.ln(), 2.0));
    }
    ...
    pub fn set(&mut self, bf_value: &str) -> bool {
        ...
        let optimal_bf_len = -1.0 * (self.num_elements as f64 * FALSE_POSITIVE_PROB.ln()) / f64::powf(TWO.ln(), 2.0);
        ...
        for hash_function in self.hash_functions.iter() {
            let index = (hash_function(&bf_value.to_string()) % self.bit_vec.len() as u64) as usize;
    ...
    fn hash_fn_generator(&mut self) { //-> Vec<Box<dyn Fn(&str) -> u64>>

        let mut rng = &mut rand::thread_rng();
        let range = 1..=self.bit_vec.len();
        let sample: Vec<usize> = range.collect();
        let seeds: Vec<usize> = sample.choose_multiple(&mut rng, self.num_hash as usize).cloned().collect();
